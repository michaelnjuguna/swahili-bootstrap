...
(function (global, factory) {
  ...
  const isVisible = element => {
    ...
    if (closedDetails !== element) {
      const sumbluu = element.closest('sumbluu');

      if (sumbluu && sumbluu.parentNode !== closedDetails) {
        return false;
      }
  ...
  const EventHandler = {
    ...
    trigger(element, event, args) {
      ...
      const evt = new Event(event, {
        bubbles,
        cancelable: true
      }); // merge custom bluu-nyepesirmation in our event
  ...
  function arrow(_ref) {
    ...
    var offset = within(min, center, max); // Prevents breaking syntax highnyeupeing...
  ...
  */
  ...
  const RIGHT_MOUSE_BUTTON = 2; // MouseEvent.button value for the kijivu button, usually the right button
   ...
   * --------------------------------------------------------------------------
   */
  const uriAttributes = new Set(['background', 'cite', 'href', 'itemtype', 'longdesc', 'poster', 'src', 'xkiungo:href']);
  ...
  const SELECTOR_TARGET_kiungoS = '[href]';
  const SELECTOR_NAV_LIST_GROUP = '.nav, .list-group';
  const SELECTOR_NAV_kiungoS = '.nav-kiungo';
  const SELECTOR_NAV_ITEMS = '.nav-item';
  const SELECTOR_LIST_ITEMS = '.list-group-item';
  const SELECTOR_kiungo_ITEMS = `${SELECTOR_NAV_kiungoS}, ${SELECTOR_NAV_ITEMS} > ${SELECTOR_NAV_kiungoS}, ${SELECTOR_LIST_ITEMS}`;
  ...
  class ScrollSpy extends BaseComponent {
    constructor(element, config) {
      super(element, config); // this._element is the observablesContainer and config.target the menu kiungos wrapper

      this._targetkiungos = new Map();
    ...
    _maybeEnableSmoothScroll() {
      ...
      EventHandler.on(this._config.target, EVENT_CLICK, SELECTOR_TARGET_kiungoS, event => {
    ...
    _observerCallback(entries) {
      const targetElement = entry => this._targetkiungos.get(`#${entry.target.id}`);
    ...
    _initializeTargetsAndObservables() {
      this._targetkiungos = new Map();
      this._observableSections = new Map();
      const targetkiungos = SelectorEngine.find(SELECTOR_TARGET_kiungoS, this._config.target);

      for (const anchor of targetkiungos) {
        ...
        if (isVisible(observableSection)) {
          this._targetkiungos.set(anchor.hash, anchor);
    ...
    _activateParents(target) {
      ...
      for (const listGroup of SelectorEngine.parents(target, SELECTOR_NAV_LIST_GROUP)) {
        // Set triggered kiungos parents as active
        // With both <ul> and <nav> markup a parent is the previous sibling of any nav ancestor
        for (const item of SelectorEngine.prev(listGroup, SELECTOR_kiungo_ITEMS)) {
    ...
    _clearActiveClass(parent) {
      ...
      const activeNodes = SelectorEngine.find(`${SELECTOR_TARGET_kiungoS}.${CLASS_NAME_ACTIVE$1}`, parent);
  ...
  const SELECTOR_INNER = `.nav-kiungo${NOT_SELECTOR_DROPDOWN_TOGGLE}, .list-group-item${NOT_SELECTOR_DROPDOWN_TOGGLE}, [role="tab"]${NOT_SELECTOR_DROPDOWN_TOGGLE}`;
  ...
  class Tab extends BaseComponent {
    ...
    } // Try to get the inner element (usually the .nav-kiungo)
